...
async function loadTeacherStudents(page = 1, limit = 10, search = "") {
  try {
    ...
    let url = `https://uni-front-zeta.vercel.app/api/etudiants?page=${page}&limit=${limit}`;
    if (search) {
...
async function loadSectionStudents(sectionId) {
  try {
    ...
    let url = `https://uni-front-zeta.vercel.app/api/sections/${sectionId}/etudiants`;
...
async function fetchSectionSchedule(sectionId) {
  try {
    ...
    /*
        const response = await fetch(
            `https://uni-front-zeta.vercel.app/api/sections/${sectionId}/schedules`,
            {
                method: "GET",
                headers: {
                    Authorization: `Bearer ${authToken}`,
    ...
    if (cachedUserData) {
      try {
        ...
        if (
          ...
          setTimeout(() => {
            fetch("https://uni-front-zeta.vercel.app/api/auth/verify", {
              method: "GET",
              headers: {
                Authorization: `Bearer ${authToken}`,
                "Content-Type": "application/json",
